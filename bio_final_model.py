...
# DESCRIPTION: Protein subcellular location prediction final model (Merged Model)
...
# 3. Shuffle the data.
# 4. Oversample training and test sets.
# 5. Shuffle training set.
# 6. Standardise training and test sets.
...
idx_X_train_res = np.array([np.arange(np.shape(X_data)[0])]).T
idx_X_train_res, Y_train_decimal_res = ROS.fit_sample(idx_X_train_res, Y_data_decimal)
X_train_seq_res = X_data_seq[idx_X_train_res[:,0], :, :]
X_train_res = X_data[idx_X_train_res[:,0], :]
X_data = 0 #save some memory
X_data_seq = 0 #save some memory
...
# Standardise the hand-crafted feature training set.
...
# Standardise the sequence representation training set.
...
history = model.fit([X_train_seq_res, X_train_res], Y_train_res,
...
# Set up blind protein data arrays.
X_blind = np.zeros((20, num_features))
X_blind_seq = np.zeros((20, num_seq_steps, num_seq_features))
#X_blind_ids = np.array((20), dtype='str')

# Import blind proteins, create hand-crafted features, and create sequence representations.
print('Importing blind proteins, creating features, and creating sequence represenations...')
fasta_sequences = SeqIO.parse(open('blind.fasta'), 'fasta')
for i, fasta_sequence in enumerate(fasta_sequences):
    #X_blind_ids[i] = fasta_sequence.id
    protein_sequence = str(fasta_sequence.seq)
    ...
    # Hand-crafted features 1-20: global amino acid composition
    X_blind[i, 0:20] = protein_aa_comp(protein_sequence)

    # Hand-crafted features 21-60: local amino acid composition
    #                              first 50 and last 50 amino acids
    if len(protein_sequence) < 100:
        if len(protein_sequence) % 2 == 0:
            X_blind[i, 20:40] = protein_aa_comp(protein_sequence[:(len(protein_sequence)) // 2])
            X_blind[i, 40:60] = protein_aa_comp(protein_sequence[(len(protein_sequence)) // 2:])
        else:
            X_blind[i, 20:40] = protein_aa_comp(protein_sequence[:(len(protein_sequence)) // 2])
            X_blind[i, 40:60] = protein_aa_comp(protein_sequence[(len(protein_sequence)) // 2 + 1:])
    else:
        X_blind[i, 20:40] = protein_aa_comp(protein_sequence[:50])
        X_blind[i, 40:60] = protein_aa_comp(protein_sequence[-50:])

    # Hand-crafted feature 61: molecular weight
    X_blind[i, 60] = protein_mw(protein_sequence)

    # Hand-crafted features 62: isoelectric point
    X_blind[i, 61] = protein_pi(protein_sequence)

    # Hand-crafted features 63: aromaticity
    X_blind[i, 62] = protein_aromaticity(protein_sequence)

    # Hand-crafted features 64: instability
    X_blind[i, 63] = protein_instability(protein_sequence)

    # Hand-crafted features 65: gravy
    X_blind[i, 64] = protein_gravy(protein_sequence)

    # Hand-crafted features 66: composition of amino acids in turn
    X_blind[i, 65] = protein_secondary_structure(protein_sequence)[1]

    # Hand-crafted features 67: composition of amino acids in sheet
    X_blind[i, 66] = protein_secondary_structure(protein_sequence)[2]
    ...
    if len(protein_sequence) < num_seq_steps:
        ...
        example_combined = np.concatenate((padding_left, example_combined, padding_right), axis=0)
        # Sequence representation: 20 one-hot features + 20 BLOSUM62 features + 1 hydrophobicity feature
        X_blind_seq[i, :, :] = example_combined

    # Else if protein at least 140 amino acids long:
    else:
        # Sequence representation: 20 one-hot features (nineteen 0s and one 1)
        X_blind_seq[i, :, 0:20] = protein_one_hot(
            protein_sequence[:(num_seq_steps//2)] + protein_sequence[-(num_seq_steps//2):])
        # Sequence representation: 20 BLOSUM62 features
        X_blind_seq[i, :, 20:40] = protein_blosum62(
            protein_sequence[:(num_seq_steps//2)] + protein_sequence[-(num_seq_steps//2):])
        # Sequence representation: 20 HSDM features
        X_blind_seq[i, :, 40:60] = protein_hsdm(
            protein_sequence[:(num_seq_steps//2)] + protein_sequence[-(num_seq_steps//2):])
        # Sequence representation: 1 hydrophobicity feature
        X_blind_seq[i, :, 60] = protein_hydro(
            protein_sequence[:(num_seq_steps // 2)] + protein_sequence[-(num_seq_steps // 2):])

# Ensure no anomalies in the form of NANs are present.
X_blind = np.nan_to_num(X_blind)
X_blind_seq = np.nan_to_num(X_blind_seq)

# Standardise the hand-crafted feature blind set.
for j in range(np.shape(X_train_res_mean)[0]):
    if X_train_res_std[j] == 0:
        X_blind[:, j] = 0
    else:
        X_blind[:, j] = (X_blind[:, j] - X_train_res_mean[j])/X_train_res_std[j]

# Standardise the sequence representation blind set.
for i in range(np.shape(X_train_seq_res_mean)[0]):
    for j in range(np.shape(X_train_seq_res_mean)[1]):
        if X_train_seq_res_std[i,j] == 0:
            X_blind_seq[:,i,j] = 0
        else:
            X_blind_seq[:,i,j] = (X_blind_seq[:,i,j] - X_train_seq_res_mean[i,j])/X_train_seq_res_std[i,j]

print('Running blind proteins through the trained model...')

preds = model.predict([X_blind_seq, X_blind])
